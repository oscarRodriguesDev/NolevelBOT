...
## 58. GRÁFICO DE FUNIL DE STATUS + EXPORTAÇÃO XLSX + COMPARATIVO ENTRE PERÍODOS (12/06/2026)

### Objetivo
Três novas funcionalidades no dashboard: gráfico de funil mostrando o fluxo de chamados por etapa (NOVO → EM_ATENDIMENTO → AGUARDANDO → CONCLUIDO), exportação avançada para Excel de todos os indicadores visíveis, e toggle de comparativo entre o período atual e o período anterior.

### 1. Gráfico de Funil — `FunnelCard.tsx`

**Arquivo:** `src/app/components/dashboard/FunnelCard.tsx` (novo)

Componente visual que exibe as etapas do chamado em formato de funil:
...
Características:
- 4 etapas no fluxo: NOVO → EM_ATENDIMENTO → AGUARDANDO → CONCLUIDO
- Largura de cada barra proporcional ao count relativo ao maior valor
- Porcentagem de retenção entre etapas (verde > 80%, amarelo < 80%, vermelho < 50%)
- Cores: Roxo (#8B5CF6) → Laranja (#F59E0B) → Rosa (#EC4899) → Verde (#10B981)
- Setas SVG entre os steps para indicar fluxo descendente
- Cada step com borda lateral colorida de 4px

### 2. API — Comparativo entre Períodos

**Arquivo:** `src/app/api/dashboards/route.ts` (modificado)

#### Funil adicionado
- `chamadosPorStatusSteps` — array ordenado com { label, count, color } para as 4 etapas
- Constante `STATUS_STEPS_ORDER` define a ordem dos steps
- Constante `STATUS_STEPS_MAP` com labels e cores

#### Date range filtering
- `getPeriodDateRange(periodo, now)` — calcula start/prevStart/prevEnd para cada período:
  | Período | Current start | Previous start | Previous end |
  |---------|---------------|----------------|--------------|
  | dia     | hoje 00:00    | ontem 00:00    | hoje 00:00   |
  | semana  | domingo 00:00 | domingo -7     | domingo 00:00|
  | mes     | 1º do mês     | 1º do mês -1   | 1º do mês    |
  | ano     | 01/01         | 01/01 -1       | 01/01        |

- `computeMetrics(empresaId, periodo, dateRange?)` — função reutilizável que computa todas as métricas para um dado range de datas
- Quando `comparar=true`, chama `computeMetrics` duas vezes (atual + anterior) e retorna `comparativo` com:
  - `atual`: métricas do período atual
  - `anterior`: métricas do período anterior
  - `variacao`: { totalGeral, tempoMedio } com % de diferença

### 3. DashboardContainer — Novas Funcionalidades

**Arquivo:** `src/app/components/dashboard/DashboardContainer.tsx` (modificado)

#### Botão "Exportar XLSX"
- Gera arquivo Excel (.xlsx) usando biblioteca `xlsx` (já existente no projeto)
- Exporta todos os indicadores disponíveis: totalGeral, tempoMedio, chamadosPorStatus, prioridade, setor, atendente, período, motivos, picoHorarios, comparativo
- Nome do arquivo: `dashboard_{modulo}_{periodo}.xlsx`
- Botão no header ao lado de "Personalizar"

#### Toggle "Comparar"
- Switch toggle no header ao lado do seletor de período
- Quando ativado:
  - API chamada com `comparar=true`
  - **KPI cards** (totalGeral, tempoMedio): subtítulo mostra `"+X% vs ant."` ou `"-X% vs ant."` com cor verde/vermelha
  - **Comparativo cards**: grid 3×2 com: Total Atual, Total Anterior, Variação Total, Variação Tempo Médio, Tempo Médio Atual, Tempo Médio Anterior
  - **Evolução Temporal**: duas séries no line chart (atual + anterior) com legenda colorida
- Funções auxiliares: `pctColor(value)` → verde (>0), vermelho (<0); `formatPct(value)` → `"+X%"`/`"-X%"`

#### Tipos atualizados
- `types.ts`: novos campos `chamadosPorStatusSteps` e `comparativo` no `DashboardData`
- Novo tipo `PeriodoKey = "dia" | "semana" | "mes" | "ano"`
- Novos indicadores: `funilStatus` e `comparativo` adicionados em `CORPORATIVO_INDICATORS` e `OFICINA_INDICATORS` (11 indicadores cada)
...
| Arquivo | Mudança |
|---------|---------|
| `src/app/api/dashboards/route.ts` | Funil + comparativo + `computeMetrics()` reutilizável + date filtering |
| `src/app/components/dashboard/DashboardContainer.tsx` | Funil, export XLSX, toggle comparativo, variação em KPI, dual line chart |
