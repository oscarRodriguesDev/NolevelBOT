...
const COLUMNS = [
  { key: "NOVO", title: "Aguardando", color: "var(--status-new)" },
  { key: "EM_ATENDIMENTO", title: "Em Andamento", color: "var(--status-in-progress)" },
  { key: "AGUARDANDO", title: "Aguardando Peças", color: "var(--status-waiting)" },
  { key: "CONCLUIDO", title: "Concluído", color: "var(--status-completed)" },
  { key: "CANCELADO", title: "Cancelado", color: "var(--status-cancelled)" },
]

// normaliza o status do chamado para uma das colunas do kanban
...
// componente de quadro kanban para arrastar e soltar chamados entre colunas
              ...
              {col.tickets.length === 0 && !loading && (
                ...
                >
                  <p className="text-xs" style={{ opacity: 0.5 }}>
                    {dragOverColumn === col.key ? "Solte aqui" : "Nenhuma"}
                  </p>
                </div>
              )}
