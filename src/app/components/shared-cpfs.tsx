...
export default function SharedCpfsPage({ setHeader, moduleSlug }: Props) {
  ...
  const isOficina = moduleSlug === "oficina"

  const [nome, setNome] = useState("")
  const [cpfValue, setCpfValue] = useState("")
  const [file, setFile] = useState<File | null>(null)
  const [registros, setRegistros] = useState<{ id?: string; nome: string; cpf: string }[]>([])
  ...
  async function fetchRegistros() {
    try {
      ...
      console.error(`Erro ao buscar ${isOficina ? "colaboradores" : "CPFs"}`, error)
    }
  }
  ...
  useEffect(() => {
    setHeader({
      titulo: isOficina ? "Cadastro de Colaboradores" : "Cadastro de CPFs",
      descricao: isOficina
        ? "Gerencie colaboradores cadastrados no sistema"
        : "Gerencie e importe CPFs autorizados",
    })
  ...
  async function handleDelete(cpfToDelete: string) {
    try {
      ...
      setRegistros(prev => prev.filter(item => item.cpf !== cpfToDelete))
      toast.success(isOficina ? "Colaborador deletado com sucesso" : "CPF deletado com sucesso")
    } catch {
      toast.error(isOficina ? "Erro ao deletar colaborador" : "Erro ao deletar CPF")
    }
  }
  ...
  async function cadastrarManual(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/cpfs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome, cpf: cpfValue })
      })
      ...
      toast.success(`${isOficina ? "Colaborador" : "CPF"} cadastrado com sucesso`)
      setNome("")
      setCpfValue("")
      fetchRegistros()
    } catch {
    ...
    >
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          >
            <div>
              <h3 className="text-lg font-semibold mb-1">
                {isOficina ? "Cadastrar CPF ou Matricula" : "Cadastrar CPF"}
              </h3>
              <p className="text-xs opacity-60 mb-6">
                {isOficina
                  ? "Adicione os colaboradores autorizados manualmente"
                  : "Adicione CPFs autorizados manualmente"}
              </p>
            </div>
            <form onSubmit={cadastrarManual} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-70">
                  {isOficina ? "Matrícula ou CPF" : "CPF"}
                </label>
                <input
                  placeholder={isOficina ? "000000" : "000.000.000-00"}
                  value={cpfValue}
        ...
        <div className="flex-1 max-w-md">
          ...
          >
            <h3 className="text-lg font-semibold mb-1">
              {isOficina ? "Colaboradores Cadastrados" : "CPFs Cadastrados"}
            </h3>
            <p className="text-xs opacity-60 mb-4">{registrosFiltrados.length} registro(s)</p>

            <input
              placeholder={isOficina ? "Buscar por Nome, cpf ou matrícula..." : "Buscar por Nome ou CPF..."}
              value={searchTerm}
              ...
              {registrosFiltrados.length > 0 ? (
                registrosFiltrados.map((item, idx) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      title={isOficina ? "Deletar Colaborador" : "Deletar CPF"}
                    >
                      <FaTrash size={12} />
                    </button>
                  </div>
                ...
                <div className="text-center py-10">
                  <p className="text-sm opacity-40 font-medium">
                    {isOficina ? "Nenhum resultado encontrado" : "Nenhum CPF encontrado"}
                  </p>
                </div>
              )}
            </div>
