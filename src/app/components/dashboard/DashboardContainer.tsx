...
import type { DashboardData, IndicatorDef } from "./types"
...
export function DashboardContainer({
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const res = await fetch(`/api/dashboards?periodo=${periodo}&modulo=${modulo}`)
        if (res.status === 403) {
          if (isMounted) setHasPermission(false)
          return
        }
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        {isEnabled("chamadosPorStatus") && (
          <div className={isEnabled("totalGeral") ? "lg:col-span-3" : "lg:col-span-4"}>
            <PieChartCard
