...
const CPFS_COLABORADORES = ["12345678901", "98765432100", "11122233344", "06230124645"]
  ...
  try {
    ...
    switch (session.state) {
      ...
      case "identificacao":
        if (!session.nome) {
          ...
          if (cleanCPF.length === 11 && CPFS_COLABORADORES.includes(cleanCPF)) {
            ...
            await sendEvolutionText(instance, number, `Poxa, não achei esse CPF aqui... 🤔 Tenta digitar novamente os 11 números para mim?`)
          }
        }
        break
        ...
        const aiResponse = await openai.chat.completions.create({
          ...
          messages: [
            { role: "system", content: `Você é a Hevelyn, assistente virtual da Nolevel. 
              Aja como uma pessoa real: seja empática, use emojis e mostre que se importa.
              Se o problema estiver no Quadro: ${QUADRO_DE_AVISOS}, explique de forma humana.
              Sempre que sugerir abrir chamado, mencione que você pode abrir por aqui ou ele pode usar o portal oficial.` },
      ...
      case "escolher_abertura":
        if (userInput.includes("1") || userInput.toLowerCase().includes("sim") || userInput.toLowerCase().includes("você")) {
