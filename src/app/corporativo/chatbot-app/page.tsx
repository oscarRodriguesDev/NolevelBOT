...
export default function MobileChat() {
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch('/api/chat-corporativo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `Enviei um arquivo: ${url}`, module: 'CORPORATIVO' })
      })
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch('/api/chat-corporativo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: text, module: 'CORPORATIVO' })
      })
      ...
      <div className="w-full max-w-3xl flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-5">
          ...
          {loading && (
            ...
            >
              <svg className="w-5 h-5 animate-spin shrink-0" style={{ color: "var(--primary)" }} fill="none" viewBox="0 0 24 24">
