...
export interface ChatConfig {
  ...
  coletarMotivoInstruction?: string
}

const defaultColetarMotivoInstruction = `Analise o motivo relatado e os avisos disponíveis seguindo as instruções do sistema.`

const detailedColetarMotivoInstruction = `Analise o motivo relatado e os avisos disponíveis seguindo as instruções do sistema. IMPORTANTE: VOCÊ DEVE retornar APENAS uma destas opções sem explicar nada além disso:
...
export async function handleChatRequest(req: NextRequest, config: ChatConfig) {
  ...
  try {
    ...
    if (!session) {
      session = {
        state: config.hasInicioFlow ? FlowState.INICIO : FlowState.IDENTIFICACAO_CPF,
        lastInteraction: Date.now(),
        modulo: moduloRequisitado || undefined,
      }
