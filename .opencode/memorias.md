...
| 2026-08-01 | F3 concluída: `MetaProvider` (Meta Cloud API) em `src/lib/whatsapp/meta-provider.ts`. `registry` agora detecta o provider por header OU heurística do payload (a Meta não permite header custom). Webhooks ganharam `GET` via `handleWebhookVerify` (handshake `hub.challenge` da Meta — valida verify_token contra empresa, 403 se inválido). Autenticação do webhook da Meta: token via `?token=` na URL (query param) ou header `x-webhook-token`; sem token → 401. Envio via Graph API `POST /{phone_number_id}/messages`, download via `GET /{media_id}` → URL assinada → bytes. 13 testes novos (279 no total), build limpo | VIBECODE |
